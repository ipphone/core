...
namespace ContactPoint
{
    public partial class MainForm : KryptonForm, IUi
    {
        ...
        private readonly UILineWrapper[] _uiLineWrappers = new UILineWrapper[5];
        //private readonly MyTranspControl _shade;

        private UILineWrapper _lastUILineWrapper;
        private ICore _core;
        ...
        public MakePhoneCallMessage CallOnStartup { get; set; }
        ...
        private bool TransferMode
        {
            ...
            set
            {
                UILineWrapper lineWrapper = GetActiveUILineWrapper();
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            _uiLineWrappers[0] = new UILineWrapper(Core, 0, btnLine1, labelCallerIDName, txtNumber, lblDuration, labelCallState, btnHold);
            _uiLineWrappers[1] = new UILineWrapper(Core, 1, btnLine2, labelCallerIDName, txtNumber, lblDuration, labelCallState, btnHold);
            _uiLineWrappers[2] = new UILineWrapper(Core, 2, btnLine3, labelCallerIDName, txtNumber, lblDuration, labelCallState, btnHold);
            _uiLineWrappers[3] = new UILineWrapper(Core, 3, btnLine4, labelCallerIDName, txtNumber, lblDuration, labelCallState, btnHold);
            _uiLineWrappers[4] = new UILineWrapper(Core, 4, btnLine5, labelCallerIDName, txtNumber, lblDuration, labelCallState, btnHold);

            _uiLineWrappers[0].Active = true;
            _lastUILineWrapper = _uiLineWrappers[0];
        ...
        void CallManager_OnCallStateChanged(ICall call)
        {
            ...
            {
                _uiLineWrappers[call.Line].Call = call;

                if (call.State == CallState.ACTIVE && !_uiLineWrappers[call.Line].Active)
                {
                    ChangeLineWrapper(_uiLineWrappers[call.Line]);
                }
            }
        ...
        {
            UILineWrapper nextLineWrapper = null;
            long lastPartTime = -1, lastPartId = -1;

            foreach (var lineWrapper in _uiLineWrappers)
                if (lineWrapper?.Call != null && !lineWrapper.Active)
                {
                    if (nextLineWrapper == null)
                    {
                        nextLineWrapper = lineWrapper;
                    }
                    ...
                    if (unid != null)
                    {
                        ...
                        if (uniqueIdParts.Length == 2)
                        {
                            ...
                            if (long.TryParse(uniqueIdParts[0], out partTime) && long.TryParse(uniqueIdParts[1], out partId))
                            {
                                ...
                                else if (nextLineWrapper == lineWrapper)
                                {
                                    lastPartTime = partTime;
                                    lastPartId = partId;
                                }
            ...
            if (nextLineWrapper != null)
            {
                ChangeLineWrapper(nextLineWrapper);
            }
        }
        ...
        void OnAccountPresenceStatusChanged(ISipAccount account)
        {
            ...
            statusLabel.Text = account.PresenceStatus?.Message ?? @"-";
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            //UILineWrapper lineWrapper = GetActiveUILineWrapper();
            //lineWrapper.Call.DoMakeConference();
        }
        ...
        private void AddCharacterToNumber(string c)
        {
            ...
            if (lineWrapper != null && lineWrapper.Call != null && lineWrapper.Call.State == CallState.ACTIVE)
                lineWrapper.Call.SendDTMF(c);
        ...
        private void OnRemoteMessageReceived(object message)
        {
            ...
            var makeCallMessage = message as MakePhoneCallMessage;
        ...
        {
            CallInternal(new MakePhoneCallMessage { Destination = number }, forceActivateLineWrapper);
        }

        private void CallInternal(MakePhoneCallMessage number, bool forceActivateLineWrapper = false)
        {
            try
            {
        ...
        private void ChangeLineWrapper(UILineWrapper targetLineWrapper)
        {
            if (_lastUILineWrapper != null && _lastUILineWrapper != targetLineWrapper && TransferMode)
            {
                if (targetLineWrapper.Call != null &&
                    targetLineWrapper.Call.State == CallState.HOLDING &&
                    _lastUILineWrapper.Call != null &&
                    (new QuestionForm(string.Format("Do you want to transfer '{0}' to '{1}'", _lastUILineWrapper.Call.Number, targetLineWrapper.Call.Number))).ShowDialog() == DialogResult.OK)
                {
                    if (targetLineWrapper.Call.State == CallState.HOLDING)
                    {
                        targetLineWrapper.Call.UnHold();
                    }

                    if (_lastUILineWrapper.Call.State == CallState.HOLDING)
                    {
                        _lastUILineWrapper.Call.UnHold();
                    }

                    targetLineWrapper.Call.TransferAttended(_lastUILineWrapper.Call);
                }

                TransferMode = false;
            }

            if (_lastUILineWrapper != null)
                _lastUILineWrapper.Active = false;

            if (targetLineWrapper != null)
            {
                _lastUILineWrapper = targetLineWrapper;
                targetLineWrapper.Active = true;
            }
        }
        ...
        private UILineWrapper GetActiveUILineWrapper()
        {
            foreach (UILineWrapper item in _uiLineWrappers)
                if (item.Active)
                    return item;

            return _uiLineWrappers[0];
        }

        private UILineWrapper GetActiveUILineWrapperByButton(KryptonCheckButton button)
        {
            foreach (UILineWrapper item in _uiLineWrappers)
        ...
        {
            MenuItemWrapper menuItem = new MenuItemWrapper(command, rootItem);
            ...
            if (childrens != null)
            {
                ...
                if (childrenItems.Items.Count > 0)
                    menuItem.Items.Add(childrenItems);
            }
            ...
            rootItem.Items.Insert(0, menuItem);
        }
